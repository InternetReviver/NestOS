...
namespace NestsOS
{
    internal class NestDosApp : UserControl
    {
        ...
        private void Execute(string commandLine)
        {
            ...
            switch (command)
            {
                case "help":
                    ...
                    WriteLine("PAINT, CALC, HOME, MEDIA, EXPORT, SETTINGS, EDIT [file], FILES, SOLITAIRE, MINES, MAHJONG, CHESS");
                    WriteLine("ANALOG, DIGITAL, USAGE, VERSION, RUN");
                    break;
                case "dir":
                    ExecuteDir();
                    break;
                ...
                case "time":
                    ...
                    break;
                case "ver":
                    WriteLine(AppInfo.BuildLabel + " single-exe desktop shell");
                    break;
                case "version":
                ...
                case "calculator":
                    host.OpenCalculator();
                    WriteLine("Launching Calculator...");
                    break;
                case "home":
                case "homepanel":
                    host.OpenHomePanel();
                    WriteLine("Launching Home Panel...");
                    break;
                case "export":
                    host.OpenExport();
                    WriteLine("Launching Export...");
                    break;
                case "media":
                case "mediacenter":
                    host.OpenMediaCenter();
                    WriteLine("Launching Media Center...");
                    break;
                case "settings":
                    host.OpenSettings();
                    WriteLine("Launching Settings...");
                    break;
                case "edit":
                    host.OpenTextEditor(parts.Count > 1 ? ResolveTextFile(parts[1], false) : null);
                ...
                case "minesweeper":
                case "winmine":
                    host.OpenMinesweeper();
                    WriteLine("Launching Minesweeper...");
                    break;
                case "mahjong":
                    host.OpenMahjong();
                ...
                case "sysusage":
                case "sysinfo":
                    host.OpenSystemUsage();
                    WriteLine("Launching System Usage...");
                    break;
                default:
        ...
        {
            var path = ResolveTextFile(requested, true);
            if (path == null)
            {
            ...
            WriteLine(NestExportCodec.ReadTextFile(path));
        }
        ...
        private string ResolveTextFile(string requested, bool mustExist)
        {
            var root = SaveSystem.EnsureRoot();
            if (Path.GetExtension(requested).Length > 0)
            {
                var explicitPath = Path.Combine(root, requested);
            ...
            var txtPath = Path.Combine(root, requested + ".txt");
            if (File.Exists(txtPath))
            {
                return txtPath;
            }

            var comwPath = Path.Combine(root, requested + ".comw");
            if (File.Exists(comwPath))
            {
                return comwPath;
            }

            return mustExist ? null : txtPath;
        }
