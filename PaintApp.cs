...
namespace NestsOS
{
    internal class PaintApp : UserControl
    {
            ...
            if (!string.IsNullOrEmpty(path))
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    catch (Exception ex)
                    {
                        currentPath = null;
                        canvas.ClearSurface();
                        host.ShowMessage("Paint", "Could not open " + Path.GetFileName(path) + ".\r\n\r\n" + ex.Message);
                    }
                }
            }
            else
        ...
        private void SaveCanvas()
        {
            ...
            var extension = currentPath != null && NestExportCodec.IsNestBitmapPath(currentPath) ? ".comu" : ".bmp";
    ...
    internal class PaintSurface : Control
    {
        ...
        {
            NestExportCodec.WriteBitmapFile(path, surface);
        }
        ...
        public void LoadSurface(string path)
        {
            ...
            using (var bmp = NestExportCodec.ReadBitmapFile(path))
            {
